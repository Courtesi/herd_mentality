...
//        log.info("User profile: {}", profile);
    ...
    @PostMapping("/delete-account/direct")
    @Operation(summary = "Direct account deletion", description = "Deletes account immediately with password verification")
    public ResponseEntity<DeleteAccountResponse> deleteAccount(@Valid @RequestBody DeleteAccountRequest request,
                                                               Authentication authentication) {
        if (authentication == null || !authentication.isAuthenticated()) {
            return ResponseEntity.status(401).body(DeleteAccountResponse.error("Authentication required"));
        }

        CustomUserPrincipal userPrincipal = (CustomUserPrincipal) authentication.getPrincipal();
        ProfileResponse profile = userService.getUserProfile(userPrincipal.getUserId());

        // Validate password requirement for email/password users
        ...
        // OAuth users should not provide password
        ...
        try {
            ...
            if (profile.getHasPassword()) {
                boolean passwordValid = userService.verifyCurrentPassword(userPrincipal.getUserId(), request.getCurrentPassword());
                if (!passwordValid) {
                    return ResponseEntity.badRequest()
                            .body(DeleteAccountResponse.error("Invalid password"));
                }
            }

            // Delete the account directly
            userService.deleteUserAccountDirect(userPrincipal.getUserId());

            log.info("Account deleted successfully for user ID: {}", userPrincipal.getUserId());
            ...
            log.error("Failed to delete account for user: {} - {}", userPrincipal.getUserId(), e.getMessage());
            return ResponseEntity.internalServerError()
                    .body(DeleteAccountResponse.error("Account deletion failed. Please try again later."));
        }
    }
